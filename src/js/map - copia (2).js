...
// 2) Flags y claves de storage
const PERSIST_TO_LOCAL_STORAGE = true; // ponlo a false si no quieres persistencia entre sesiones
const LS_KEYS = {
    main: 'vista_inicial_main',
    can:  'vista_inicial_canarias'
};

// 3) Variables runtime para la vista inicial (se actualizan al guardar)
...
function getCurrentView(mapInstance) {
    return {
        center: mapInstance.getCenter().toArray(), // [lng, lat]
...
function setInitialView(mapInstance, view, target = 'main') {
    if (target === 'main') {
        ...
        if (PERSIST_TO_LOCAL_STORAGE) {
            localStorage.setItem(LS_KEYS.main, JSON.stringify(initialViewMain));
        }
        console.log('💾 Vista inicial MAIN guardada:', initialViewMain);
    } else {
        initialViewCan = { ...view };
        if (PERSIST_TO_LOCAL_STORAGE) {
            localStorage.setItem(LS_KEYS.can, JSON.stringify(initialViewCan));
        }
        console.log('💾 Vista inicial CANARIAS guardada:', initialViewCan);
    }
}
function resetToInitialView(mapInstance, target = 'main', { animate = true } = {}) {
    const view = target === 'main' ? initialViewMain : initialViewCan;
    applyView(mapInstance, view, { animate });
    console.log(`↩️ Restaurada vista inicial (${target})`);
}
...
// 5) Cargar de localStorage al arrancar (si existen)
...
// 6) Hooks de teclado para rápido en auditorio:
//    Ctrl+Shift+S -> guarda ambos como inicial
//    Ctrl+Shift+R -> restaura ambos (flyTo)
function setupKeyboardShortcuts() {
    window.addEventListener('keydown', (ev) => {
        ...
        if (isSave) {
            ...
            resetToInitialView(map, 'main', { animate: true });
            resetToInitialView(canariasMap, 'canarias', { animate: true });
            ev.preventDefault();
        }
    });
}

// 7) Eventos públicos para integrarte con tu UI/QR:
//    window.dispatchEvent(new CustomEvent('vista-guardar', { detail: { target: 'main'|'canarias' } }))
//    window.dispatchEvent(new CustomEvent('vista-restaurar', { detail: { target: 'main'|'canarias', animate: true } }))
function setupViewEvents() {
    ...
    window.addEventListener('vista-restaurar', (e) => {
        ...
        const mapInst = target === 'main' ? map : canariasMap;
        resetToInitialView(mapInst, target, { animate });
    });
}

// 8) Inicialización (llámalo tras el 'load' de cada mapa)
function initViews() {
    loadInitialViewsFromStorage();
    setupKeyboardShortcuts();
    setupViewEvents();

    // Opcional: aplicar la vista inicial en el arranque
    applyView(map, initialViewMain, { animate: false });
    applyView(canariasMap, initialViewCan, { animate: false });
}

// Llama a initViews cuando ambos mapas estén listos:
Promise.all([
