...
import { loadActivosDesdeJson } from './js/map.js';
...
window.addEventListener('load', () => {
    fetch(PRIMARY_URL)
        ...
        .catch(async (err) => {
            console.warn('[main] fallo en primaria → probamos fallback:', PRIMARY_URL, '→', FALLBACK_URL, err);
            try {
...
// Enciende en frío las activas cuando ambos mapas están listos
window.addEventListener('maps:ready', async () => {
    console.log('[main] maps:ready → encendemos activos en frío');
    try {
        await loadActivosDesdeJson(PRIMARY_URL);
    } catch (e1) {
        console.warn('[main] loadActivosDesdeJson falló con primaria; probamos fallback:', e1);
        try {
            await loadActivosDesdeJson(FALLBACK_URL);
        } catch (e2) {
            console.error('[main] Error cargando activos (primaria y fallback):', e2);
        }
    }
});
